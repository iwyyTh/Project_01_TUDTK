...
\newpage
\listoffigures % Tạo danh mục hình ảnh tự động
...
\subsubsection{Cơ sở lý thuyết}
Phân rã SVD biến đổi ma trận $\mathbf{A}_{m \times n}$ thành tích của ba ma trận: $\mathbf{A} = \mathbf{U}\Sigma\mathbf{V}^T$. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Đồ thị Log-Log Thời gian thực thi vs Kích thước ma trận (n).png}
    \caption{Đồ thị Log-Log so sánh thời gian thực thi giữa các phương pháp Gauss, Gauss-Seidel và SVD.}
    \label{fig:benchmark_loglog}
\end{figure}

\textbf{Nhận xét từ đồ thị:}
\begin{itemize}
    \item \textbf{Khử Gauss (Partial Pivot):} Đường biểu diễn (màu xanh dương) bám rất sát đường lý thuyết $O(n^3)$ (nét đứt đen), xác nhận độ phức tạp thuật toán cài đặt đúng với kỳ vọng.
    \item \textbf{SVD (Pseudoinverse):} Mặc dù cùng độ phức tạp bậc 3, nhưng SVD (đường màu xanh lá) có hệ số hằng số thấp hơn trong các bài thử nghiệm ma trận nhỏ và trung bình, tuy nhiên sẽ tăng mạnh khi kích thước ma trận đạt ngưỡng rất lớn.
    \item \textbf{Gauss-Seidel:} Đường biểu diễn (màu cam) nằm thấp nhất, cho thấy tốc độ vượt trội đối với các ma trận chéo trội nhờ bản chất lặp và cập nhật tại chỗ (\textit{in-place update}).
\end{itemize}

\subsubsection{Tính ổn định số và Số điều kiện (Numerical Stability)}
Sự tương quan giữa cấu trúc ma trận và sai số được thể hiện rõ qua đồ thị so sánh sai số tương đối giữa hai kịch bản: ma trận SPD (ổn định) và ma trận Hilbert (điều kiện xấu).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{So sánh sai số tương đối Ma trận Hilbert vs Ma trận SPD.png}
    \caption{So sánh sai số tương đối trên thang Log giữa ma trận Hilbert và ma trận SPD.}
    \label{fig:error_comparison}
\end{figure}

\textbf{Phân tích từ đồ thị:}
\begin{itemize}
    \item \textbf{Với ma trận SPD (đường nét đứt):} Cả ba phương pháp đều duy trì sai số ổn định ở mức rất thấp ($10^{-10} \sim 10^{-15}$). Điều này chứng tỏ khi ma trận có số điều kiện tốt, các thuật toán nhóm cài đặt đạt độ chính xác tiệm cận sai số máy.
    \item \textbf{Với ma trận Hilbert (đường nét liền):} 
        \begin{itemize}
            \item Sai số tăng vọt theo kích thước $n$. Đặc biệt, phương pháp \textbf{Gauss-Seidel} gần như mất khả năng hội tụ (sai số xấp xỉ $10^0 = 1$) do ma trận Hilbert không thỏa điều kiện chéo trội.
            \item \textbf{SVD (Pseudoinverse)} duy trì sai số thấp hơn so với Gauss khi $n$ tăng, cho thấy ưu thế của việc lọc các trị kỳ dị nhỏ trong việc ổn định hóa nghiệm của hệ phương trình điều kiện xấu.
        \end{itemize}
\end{itemize}
...
\begin{table}[H]
    ...
    \textbf{Thành viên} & \textbf{MSSV} & \textbf{Nội dung công việc phụ trách} & \textbf{Đánh giá} \\ \hline
...
\begin{itemize}[leftmargin=*, nosep]
    \item Part 2: \texttt{diagonalization.py}
    \item Part 3: \texttt{solvers.py}
    \item Viết báo cáo.
\end{itemize} & 100\% \\ \hline
Trương Bảo Nguyên & 24120397 & 
\begin{itemize}[leftmargin=*, nosep]
    \item Part 2: \texttt{decomposition.py}
...
\end{itemize} & 100\% \\ \hline
    \end{tabularx}
    \caption{Bảng phân công nhiệm vụ và đánh giá mức độ hoàn thành}
\end{table}
