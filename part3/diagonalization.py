...
def compute_eigenvalues(A):
    ...
    Luôn trả về (eigvals, V):
      - n >= 5: eigvals (ndarray 1D), V (ndarray n×n), cột j là vector riêng ứng với eigvals[j]
      - n < 5:  eigvals (list), V là None (vector riêng do diagonalize() tính qua compute_eigenspace)
    """
    n = len(A)

    if n >= 5:
        print(f"  [Info] n={n} >= 5: Sử dụng NumPy (np.linalg.eig)...")
        eigvals, eigvecs = np.linalg.eig(np.array(A, dtype=float))
        return eigvals, eigvecs

    if n == 4:
        print(f"  [Info] n={n} == 4: Kích hoạt Lặp QR Iteration nội bộ (Bỏ qua định lý Abel)...")
        return compute_eigenvalues_qr(A), None
...
def diagonalize(A):
    ...
    eigvals_raw, V = compute_eigenvalues(A)

    # n >= 5: trị riêng + vector riêng từ NumPy (cột j của V ứng với eigvals[j])
    if V is not None:
        w = np.asarray(eigvals_raw, dtype=complex)
        P_np = np.asarray(V, dtype=complex)
        if np.max(np.abs(w.imag)) > 1e-8:
            print("  [Cảnh báo] Trị riêng phức → không chéo hóa được trên ℝ")
            ev_out = [float(z.real) if abs(z.imag) < 1e-10 else complex(z) for z in w]
            return False, None, None, None, ev_out, []
        if np.max(np.abs(P_np.imag)) > 1e-8:
            print("  [Cảnh báo] Vector riêng phức → không chéo hóa được trên ℝ")
            return False, None, None, None, np.real(w).tolist(), []
        w = np.real(w)
        P_np = np.real(P_np)
        D_np = np.diag(w)
        try:
            ...
            return False, None, None, None, w.tolist(), []
        A_np = np.array(A, dtype=float)
        recon = P_np @ D_np @ P_inv_np
        err = np.max(np.abs(A_np - recon))
        tol = 1e-5 * max(1.0, np.linalg.norm(A_np, ord="fro"))
        is_diagonalizable = err < tol
        P_list = P_np.tolist()
        D_list = D_np.tolist()
        P_inv_list = P_inv_np.tolist()
        P_cols = [[P_np[i, j] for i in range(n)] for j in range(n)]
        return is_diagonalizable, P_list, D_list, P_inv_list, w.tolist(), P_cols

    eigenvalues = eigvals_raw
